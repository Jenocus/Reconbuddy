...
st.write("- 📊 Review matched, unmatched, and all rows in frozen-header, sortable tables")
st.write("- ✏️ Edit and rate mismatch reasons with inline 👍 (confirm) and 👎 (flag wrong) per row")
st.write("- 📚 Learn from user feedback: confirmed reasons train the LLM; flagged reasons are banned from future runs")
st.write("- ⏰ Automatically detect timing differences by comparing transaction dates to matched period (highest priority)")
st.write("- ✓ Admin panel to manage confirmed pairings, user examples, and flagged reasons")
st.write("- 💬 Ask questions about sources and reconciliation results via LLM chatbox")
st.write("- 🤖 LLM-based reasoning for field interpretation and unmatched reason inference")
